...
Client class
 */
export class udireccionElement {
  ...
  static clone(client: udireccionElement) {
    return new udireccionElement(client.direccion,
      client.cedula,);
  }
}
 ...
 */
export class UDireccionesComponent {
  ...
  public get_Direccions() {
    var res = this.http.get<string>("https://localhost:7274/api/Client/Direccion/list/" + this.cedula, {
      headers: this.httpOptions.headers,
      withCredentials: true
    }).subscribe(result => {
      console.log(this.respuesta);
      this.Direcciones = <udireccionElement[]><unknown>result;
  ...
  async Add() {
    ...
    console.log(this.respuesta);
    console.log(answer);
    let res = await this.http.put("https://localhost:7274/api/Client/Direccion/add", JSON.stringify(answer), {
        headers: this.httpOptions.headers,
        withCredentials: true,
      }
    )
   ...
   */
  async Delete_Button(Client: udireccionElement
  ) {
    Popup.open("Eliminar Direccion", "Desea Eliminar este Direccion?", "Sí",
      (context: UDireccionesComponent = this) => () => context.delete_Worker([String(Client.cedula), String(Client.direccion)]))
  }

  async delete_Worker(key: string[]
  ) {
    console.log("Direccion eliminado: " + (key[0]))
    let res = await this.http.delete("https://localhost:7274/api/Client/Direccion/delete", {
        headers: this.httpOptions.headers,
